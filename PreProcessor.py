...
class PreProcessor:
    ...
    def process():
        ...
        train_df = pd.read_csv(TRAINING_INPUT_PATH, names=INPUT_COLUMN_NAMES)
        test_df = pd.read_csv(TEST_INPUT_PATH, names=INPUT_COLUMN_NAMES)
    ...
    def clean_input_df(train_df, test_df):
        ...
        print_df(train_df, name="clean_train_df")
        print_df(test_df, name="clean_test_df")
    ...
    def one_hot_encoding(input_df):
        print_df(input_df, name= "encoding_input_df")
        encoded_df = pd.get_dummies(input_df)
        print_df(encoded_df, name="encoding_output_df")
        return encoded_df
